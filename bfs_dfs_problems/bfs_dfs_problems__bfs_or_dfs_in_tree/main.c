...
void	dfs(AdList **G, int *visited, int position, int Y) {
	visited[position] = 1;
	if (visited[Y] == 0) {
		dfs_count += 1;
	}
...
int	main(void) {
	...
	for (int i = 1; i <= N; i++) {
		if (adlist[i] != NULL) {
			qsort(adlist[i]->arr, adlist[i]->len, sizeof(int), cmp_asc);
		}
	}

	int *visited_dfs = calloc(N+1, sizeof(int));
	dfs(adlist, visited_dfs, X, Y);

	Queue *que = new_queue(2);
	enqueue(que, X);

	int *visited_bfs = calloc(N+1, sizeof(int));
	for (int i = 1; i <= N; i++) {
		visited_bfs[i] = -1;
	}
	visited_bfs[X] = 0;
